// ===============================
// الأكواد الخاصة بالهجمات، تسجيل الدخول، تسجيل حساب، والكويز
// ===============================
...
(function() {
  ...
  // --------------------------
  // Form validation (Sign Up & Sign In)
  // --------------------------
    ...
    if (signupForm) {
      ...
      if (passwordInput && strengthBar) {
        passwordInput.addEventListener('input', function() {
          ...
          if (strength <= 2) strengthBar.style.backgroundColor = '#ff6b6b';
          else if (strength <= 3) strengthBar.style.backgroundColor = '#ffd93d';
          else strengthBar.style.backgroundColor = '#48bb78';
        });
      }
    }
  ...
  function initQuiz() {
    if (submitQuizBtn && quizForm) {
      submitQuizBtn.addEventListener('click', function() {
        document.querySelectorAll('.correct-answer, .incorrect-answer')
          .forEach(el => el.classList.remove('correct-answer', 'incorrect-answer'));
        let score = 0;
    ...
    function checkAnswer(questionNumber, selectedValue) {
      const correctAnswers = { 1: 'a', 2: 'b', 3: 'c', 4: 'b', 5: 'a' };
    ...
    function showQuizResults(score, total) {
      ...
      let feedback = '';
      if (percentage >= 80) feedback = "ممتاز! لديك معرفة ممتازة بالأمن السيبراني.";
      else if (percentage >= 60) feedback = "جيد جدًا! يمكنك تحسينها أكثر.";
      else if (percentage >= 40) feedback = "متوسط. راجع قسم الحماية.";
      else feedback = "تحتاج إلى المزيد من التعلم.";
